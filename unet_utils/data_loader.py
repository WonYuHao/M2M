...
def _is_image_file(name: str) -> bool:
    lower = name.lower()
    return lower.endswith('.jpg') or lower.endswith('.jpeg') or lower.endswith('.png') or lower.endswith('.bmp') or lower.endswith('.webp')


def _safe_int_prefix(filename: str):
...
class MVTecDRAEMTestDataset_partial(Dataset):
    ...
    def __init__(self, root_dir, resize_shape=None):
        ...
        for _, anomaly_name in enumerate(self.anomaly_names):
            ...
            img_files = [f for f in os.listdir(img_path) if _is_image_file(f)]
            img_files = _sort_files(img_files)
            l = len(img_files) // 3
...
class MVTec_Anomaly_Detection(Dataset):
    def __init__(self, args, sample_name, length=5000, anomaly_id=None, recon=False):
        ...
        self.good_files = [os.path.join(self.good_path, i) for i in os.listdir(self.good_path) if _is_image_file(i)]

        # 严格使用你的当前格式：
        # {generated_data_path}/{class}/generated_datasets/{anomaly}/000.png
        # {generated_data_path}/{class}/mask/{anomaly}/000_mask.png
        self.root_dir = '%s/%s' % (args.generated_data_path, sample_name)
        generated_root = os.path.join(self.root_dir, 'generated_datasets')
        mask_root = os.path.join(self.root_dir, 'mask')

        if not os.path.isdir(generated_root) or not os.path.isdir(mask_root):
            raise FileNotFoundError(
                f'Expected generated format not found: {generated_root} and {mask_root}'
            )

        self.anomaly_names = [d for d in os.listdir(generated_root) if os.path.isdir(os.path.join(generated_root, d))]
        ...
        for anomaly in self.anomaly_names:
            img_path = []
            mask_path = []

            img_dir = os.path.join(generated_root, anomaly)
            msk_dir = os.path.join(mask_root, anomaly)
            if os.path.isdir(img_dir) and os.path.isdir(msk_dir):
                img_files = [f for f in os.listdir(img_dir) if _is_image_file(f)]
                img_files = _sort_files(img_files)

                for imf in img_files[:500]:
                    stem = os.path.splitext(imf)[0]
                    mmf = f'{stem}_mask.png'
                    mpath = os.path.join(msk_dir, mmf)
                    if os.path.exists(mpath):
                        img_path.append(os.path.join(img_dir, imf))
                        mask_path.append(mpath)
    ...
    def __getitem__(self, idx):
        if random.random() > 0.5:
            ...
            # 保证抽到有数据的 anomaly 子类
            max_try = self.anomaly_num
            while len(self.mask_paths[anomaly_id]) == 0 and max_try > 0:
                anomaly_id = random.randint(0, self.anomaly_num - 1)
                max_try -= 1
            if len(self.mask_paths[anomaly_id]) == 0:
                image = self.loader(Image.open(self.good_files[idx % len(self.good_files)]).convert('RGB'))
                mask = torch.zeros((1, image.size(-2), image.size(-1)))
                has_anomaly = np.array([0], dtype=np.float32)
                sample = {'image': image, 'has_anomaly': has_anomaly, 'mask': mask, 'anomay_id': -1}
...
class MVTec_classification_test(Dataset):
    def __init__(self, args, sample_name, anomaly_names):
        ...
        for idx, anomaly_name in enumerate(self.anomaly_names):
            ...
            self.labels += [idx for _ in img_files[l:]]
        self.loader = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize([256, 256])
        ])
